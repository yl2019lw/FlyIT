...
class Transformer(nn.Module):
    def __init__(self, fvdim=512, dropout=0.1,
                 NUM_HEADS=4, NUM_LAYERS=3, k=10):
        ...
        self.proj = nn.Linear(MODEL_DIM, k)
...
class E2ETransformer(nn.Module):
    ...
    def forward(self, x, nslice):
        nsample = x.shape[0]
        max_s = max(nslice)
        inputs = torch.zeros((nsample, max_s, self.fvdim)).cuda()
        for s in range(nsample):
            fvs = self.fvextractor(x[s, :nslice[s], :])
            inputs[s][:nslice[s], :] = fvs
